...
app.get([
  `/download/${TIER_TAGGER_JAR_NAME}`,
  `/downloads/${TIER_TAGGER_JAR_NAME}`,
  `/${TIER_TAGGER_JAR_NAME}`,
], serveTierTaggerJar);
app.get('/api/tiertagger/stats', (req, res) => {
  res.json({
    totalDownloads: loadTierTaggerDownloads(),
    fileExists: Boolean(findTierTaggerJar()),
    fileName: TIER_TAGGER_JAR_NAME,
  });
});
app.use(express.static(path.join(__dirname, 'public')));


// ── WEBSITE ROUTES + TIER TAGGER DOWNLOAD TRACKING ───────────────────────────
...
const TIER_TAGGER_JAR_CANDIDATES = [
  ...
  path.join(__dirname, 'public', 'downloads', TIER_TAGGER_JAR_NAME),
  path.join(__dirname, TIER_TAGGER_JAR_NAME),
  path.join(DATA_DIR, TIER_TAGGER_JAR_NAME),
];

function getIndexFilePath() {
  return INDEX_FILE_CANDIDATES.find(fs.existsSync) || null;
}

function sendIndexPage(req, res) {
  const indexFile = getIndexFilePath();
  if (!indexFile) return res.status(404).send('index.html not found');
  return res.sendFile(indexFile);
}

function loadTierTaggerDownloads() {
  try {
    if (fs.existsSync(TIER_TAGGER_DOWNLOAD_FILE)) {
      const parsed = JSON.parse(fs.readFileSync(TIER_TAGGER_DOWNLOAD_FILE, 'utf8'));
      return Number(parsed?.totalDownloads) || 0;
    }
  } catch (_) {}
  return 0;
}

function saveTierTaggerDownloads(totalDownloads) {
  try {
    const dataDir = path.join(__dirname, 'paktiers_data');
    if (!fs.existsSync(dataDir)) fs.mkdirSync(dataDir, { recursive: true });
    fs.writeFileSync(TIER_TAGGER_DOWNLOAD_FILE, JSON.stringify({ totalDownloads }, null, 2));
  } catch (_) {}
}

function incrementTierTaggerDownloads() {
  const next = loadTierTaggerDownloads() + 1;
  saveTierTaggerDownloads(next);
  return next;
}

function findTierTaggerJar() {
  return TIER_TAGGER_JAR_CANDIDATES.find(fs.existsSync) || null;
}

function serveTierTaggerJar(req, res) {
  const jarPath = findTierTaggerJar();
  if (!jarPath) {
    return res.status(404).json({
      error: 'TierTagger jar not found',
      expectedFile: TIER_TAGGER_JAR_NAME,
    });
  }

  incrementTierTaggerDownloads();
  return res.download(jarPath, TIER_TAGGER_JAR_NAME, (err) => {
